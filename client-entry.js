...
function CalendarSummary() {
    ...
    return react.createElement('div', { style: { border: '1px solid #ccc', padding: '1em', borderRadius: '8px', overflowX: 'auto' } }, react.createElement('div', { style: { minWidth: '480px' } }, react.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5em', marginBottom: '0.5em' } }, react.createElement('button', { onClick: () => setYearMonth(shiftMonth(yearMonth, -1)) }, '<'), react.createElement('button', { onClick: () => setYearMonth(shiftMonth(yearMonth, +1)) }, '>'), react.createElement('strong', {}, `${year}年${parseInt(month)}月`), react.createElement('span', { style: { fontSize: '0.8em', color: '#888' } }, `(回答者数: ${aggregate.totalResponders}人)`)), react.createElement('table', { style: { width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' } }, react.createElement('thead', {}, react.createElement('tr', {}, WEEKDAY_LABELS.map((label) => react.createElement('th', { key: label, style: { padding: '4px', width: '14.28%' } }, label)))), react.createElement('tbody', {}, weeks.map((week, wi) => react.createElement('tr', { key: wi }, week.map(cell => {
        ...
        }, react.createElement('div', { style: { fontWeight: 'bold' } }, cell.day), (eventsByDate[cell.date] ?? []).map((title, i) => react.createElement('div', { key: i, style: { fontSize: '0.65em', background: '#e0f0ff', borderRadius: '4px', padding: '1px 3px', marginTop: '2px' } }, title)));
    }))))), 
    // ---- 選択した日の詳細パネル ----
    selectedDate != null ? react.createElement('div', { style: { marginTop: '0.8em', padding: '0.8em', background: '#fafafa', border: '1px solid #ddd', borderRadius: '8px' } }, react.createElement('strong', {}, `${selectedDate} の出欠状況`), selectedAgg == null || (selectedAgg.yes.length === 0 && selectedAgg.maybe.length === 0 && selectedAgg.no.length === 0)
        ...
        : react.createElement('div', { style: { marginTop: '0.5em', fontSize: '0.9em' } }, selectedAgg.yes.length > 0 ? react.createElement('div', { style: { color: '#2e7d32' } }, `○ 参加可能: ${selectedAgg.yes.join('、')}`) : null, selectedAgg.maybe.length > 0 ? react.createElement('div', { style: { color: '#a68b00' } }, `△ 未定: ${selectedAgg.maybe.join('、')}`) : null, selectedAgg.no.length > 0 ? react.createElement('div', { style: { color: '#c62828' } }, `× 不可: ${selectedAgg.no.join('、')}`) : null)) : react.createElement('p', { style: { marginTop: '0.8em', fontSize: '0.85em', color: '#888' } }, '日付をタップすると、その日の詳しい出欠状況が見られます')));
}
async function fetchAvailabilityAggregate(yearMonth) {
    const res = await fetch('/_api/v3/pages/list?path=' + encodeURIComponent(`/schedule/responses/${yearMonth}`), { credentials: 'include' });
    const listData = await res.json();
    ...
    for (const p of pages) {
        ...
        for (const [date, state] of Object.entries(data)) {
            perDate[date] ?? (perDate[date] = { yes: [], maybe: [], no: [] });
            if (state === 'yes')
                perDate[date].yes.push(username);
            else if (state === 'maybe')
                perDate[date].maybe.push(username);
            else if (state === 'no')
                perDate[date].no.push(username);
        }
    }
